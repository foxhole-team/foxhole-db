...
What is mirrored: Android package identifiers, certificate fingerprints (SHA-256 and SHA-1, in separate fields), and the C2 and distribution domains and IP addresses from the same upstream records. They are reformatted into the `sentinel-threat-intel-json` structure the app reads. Not mirrored: vendor websites, YARA rules, APK digests, iOS bundle identifiers and X.509 subject matchers. No upstream content is modified in substance; the exact upstream commit and input digest are recorded in `threat-intel-source-info.json` so the derivation stays auditable.
